...
<script>
  ...
  // When true, the avatar stretches to the size of its parent container.
  ...
  $: normalizedSize = Number(displaySize);
  $: hasNumericSize = Number.isFinite(normalizedSize);
  $: widthStyle = fill ? '100%' : (hasNumericSize ? `${normalizedSize}px` : String(displaySize));
  $: heightStyle = fill ? '100%' : (hasNumericSize ? `${normalizedSize}px` : String(displaySize));
  $: borderRadiusStyle = mergedOpts.rounded === false ? '0' : '50%';
</script>
...
<img
  ...
  style="width: {widthStyle}; height: {heightStyle}; border-radius: {borderRadiusStyle};"
  loading="lazy"
/>
