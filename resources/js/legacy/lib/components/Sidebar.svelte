...
<script>
  import { page } from '@/legacy/app/stores';
  ...
  const topMenu = [
    ...
    // These modules stay visible in navigation, but the pages run in preview mode
    // until their backend contracts are fully ready.
    { icon: 'users', label: 'Teams', href: '/workspace/teams', locked: true },
    { icon: 'message-square', label: 'Chat', href: '/workspace/chat' },
    { icon: 'check-square', label: 'Tasks', href: '/workspace/tasks', locked: true },
    { icon: 'calendar', label: 'Calendar', href: '/workspace/calendar', locked: true },
    { icon: 'folder', label: 'Shared File System', href: '/workspace/files', locked: true },
    { icon: 'fan', label: 'AI', href: '/workspace/ai', locked: true },
  ];

  const bottomMenu = [
    // Profile route is backed by UserController::profile (Inertia profile/show).
  ...
  function animate(time) {
    ...
    const isAIPage = $page.url.pathname === '/workspace/ai';
    let speed;
    if (isAIPage) {
  ...
  function isPreviewFeature(item) {
