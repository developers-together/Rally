<script>
  ...
  async function fetchTeams() {
    try {
      const data = await api('/user/teams');
      teams = (data || []).map(t => ({
  ...
  async function confirmLeaveTeam(id) {
    try {
      await api(`/team/${id}/delete`, { method: 'DELETE' });
  ...
  async function joinTeam() {
    ...
    try {
      await api('/team/joinTeam', { method: 'POST', body: { code: joinCode.toUpperCase() } });
      fetchTeams();
  ...
  async function createTeam() {
    ...
    try {
      await api('/team/create', {
        method: 'POST',
        body: { name: teamName, projectname: projectName, description: teamDescription, code: newCode },
      });
      teams = [...teams, { id: teams.length + 1, name: teamName, projectname: projectName, description: teamDescription, code: newCode }];
    ...
    {#each teams as team (team.id)}
      <div class="team-card" on:click={() => goToDashboard(team.id)}>
        <div class="team-avatar-container">
          <Avatar name={team.name} options={{ size: '200', background: '0052d4', color: 'fff', bold: true, rounded: false }} className="team-avatar" />
...
<style>
  ...
  .team-avatar-container { position: relative; height: 50%; }
  :global(.team-avatar) { width: 100%; height: 100%; object-fit: cover; border-radius: 8px; }
